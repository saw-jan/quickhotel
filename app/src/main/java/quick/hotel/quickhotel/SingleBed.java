...
public class SingleBed extends AppCompatActivity implements DatePickerDialog.OnDateSetListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        StringRequest request = new StringRequest(Request.Method.POST, Path.hsingle,
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        ...
                        if(!result.equals("invalid")) {
                            ...
                            progressDialog.dismiss();
                            Toast.makeText(SingleBed.this, "Cannot load details", Toast.LENGTH_LONG).show();
                        }
                    }
            ...
            public void onErrorResponse(VolleyError error) {
                progressDialog.dismiss();
                Toast.makeText(SingleBed.this, "Connection Error"+error, Toast.LENGTH_LONG).show();
            }
        }) {
            @Override
    ...
    public void bookRoom(View v) {
        ...
        if (!person.equals("")){
            if (!passW.equals("")) {
                ...
                progressDialog = new ProgressDialog(SingleBed.this);
                ...
                StringRequest request = new StringRequest(Request.Method.POST, Path.book,
                        new Response.Listener<String>() {
                            ...
                            public void onResponse(String response) {
                                ...
                                if (!result.equals("invalid")) {
                                    progressDialog.dismiss();
                                    Toast.makeText(SingleBed.this, "Booking Made", Toast.LENGTH_LONG).show();

                                    prefs.edit().remove("chkin").apply();
                                    prefs.edit().remove("chkout").apply();
                                    prefs.edit().remove("person").apply();
                                    //call dash.
                                    Intent call = new Intent(SingleBed.this, Dash.class);
                                    startActivity(call);
                                    finish();

                                } else {
                                    progressDialog.dismiss();
                                    Toast.makeText(SingleBed.this, "Invalid Password. Please Try Again !", Toast.LENGTH_LONG).show();
                                }
                            }
                        }, new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        progressDialog.dismiss();
                        Toast.makeText(SingleBed.this, "Network Error", Toast.LENGTH_LONG).show();
                    }
                }) {
                    @Override
                    protected Map<String, String> getParams() {
                ...
                RequestQueue requestQueue = Volley.newRequestQueue(SingleBed.this);
                requestQueue.add(request);
            } else {
                cPass.setFocusable(true);
                Toast.makeText(SingleBed.this, "Password Required", Toast.LENGTH_LONG).show();
            }
        }else {
            cPass.setFocusable(true);
            Toast.makeText(SingleBed.this, "People filed is empty", Toast.LENGTH_LONG).show();
        }
        }else {
            cPass.setFocusable(true);
            Toast.makeText(SingleBed.this, "Please set Check-in/Check-out date", Toast.LENGTH_LONG).show();
        }
    }
    @Override
    ...
    public void onBackPressed() {
        Intent back = new Intent(SingleBed.this, Dash.class);
        startActivity(back);
        finish();
    }
